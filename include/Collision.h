...
public:
  ...
  void AssociateCell(Cell* Cell) noexcept { cell = Cell; }
  ...
  void DetectHollowBoxCollImage(const double Image_Coef,
                                const int Reserve) noexcept;

  void DetectCylinderColl(const int Reserve) noexcept;

  void DetectCylinderCollImage(const double Image_Coef,
                               const int Reserve) noexcept;

  void DetectHollowCylinderColl(const int Reserve) noexcept;

  void DetectHollowCylinderCollImage(const double Image_Coef,
                                     const int Reserve) noexcept;

  void DetectSphereColl(const int Reserve) noexcept;

  void DetectSphereCollImage(const double Image_Coef,
                             const int Reserve) noexcept;

  void DetectHollowSphereColl(const int Reserve) noexcept;

  void DetectHollowSphereCollImage(const double Image_Coef,
                                   const int Reserve) noexcept;
  ...
  std::pair<double, Vec3> GetOverlapVec(const double Dist_Sq,
                                        ...
                                        const Vec3& Rel_Pos) const noexcept
  {
  ...
  void ClampPos1D(bool& is_Inside_Box, double& O_Coll_Pt, const double Min,
  ...
  bool ClampPos(Vec3& O_Coll_Pt, const Vec3& Min, const Vec3& Max) const
    noexcept
  {
  ...
  std::pair<int, double> GetCloseDirDist(const Vec3& Ptcl, const Vec3& Box_Min,
                                         const Vec3& Box_Max) const noexcept
  {
    const std::array<double, 6> list{Ptcl.x - Box_Min.x, Box_Max.x - Ptcl.x,
  ...
  double GetDiffCloseDir(const int Dir, const Vec3& Ptcl, const Vec3& Box_Min,
                         const Vec3& Box_Max) const noexcept
  {
    const std::array<double, 6> list{
      Ptcl.x - Box_Min.x, Box_Max.x - Ptcl.x, Ptcl.y - Box_Min.y,
      Box_Max.y - Ptcl.y, Ptcl.z - Box_Min.z, Box_Max.z - Ptcl.z,
    };
    return list[Dir];
  }
  ...
  {
    constexpr std::array<Vec3, 6> list{Vec3::UnitX(), -Vec3::UnitX(),
                                       Vec3::UnitY(), -Vec3::UnitY(),
                                       Vec3::UnitZ(), -Vec3::UnitZ()};
    return list[Dir];
  }

  double GetImageForceLen(const double Dist, const double Coef,
                          const double ChgSq) const noexcept
  {
  ...
  std::pair<double, Vec3> GetJnLen(const Vec3& Pt_Velo, const Vec3& U_N_Vec,
                                   const double Coef_Rest_N,
                                   const double M_Inv_Sum_Inv) const noexcept
  {
            ...
            const double Jt_Len, const Vec3& U_T_Vec) const noexcept
  {
    ...
    const Vec3 Jt = (Fri_Len > Jt_Len) ? -Jt_Len * U_T_Vec : -Fri_Len * U_T_Vec;
    return Jn + Jt;
  }

  Vec3 GetRollFri(const Vec3& Ang_Velo, const double Jn_Len,
                  const double Coef) const noexcept
  {
    ...
    return (roll_fri >= ang_velo_len) ? Ang_Velo
                                      : roll_fri * Ang_Velo / ang_velo_len;
  }
};
