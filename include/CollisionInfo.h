...
{
  // object: opponent object to collide with�@
  ...
  CollisionInfo(const int Type, const int ID_A, const int ID_B,
                const double Time, const double Overlap, const Vec3& Vec,
                const Vec3& H_Step_Velo_B) noexcept
    : obj_type(Type),
      id_A(ID_A),
      id_B(ID_B),
      coll_time(Time),
  ...
  bool operator<(const CollisionInfo& VS) const noexcept
  {
    ...
    return (std::abs(coll_time - VS.coll_time) > e)
             ? coll_time < VS.coll_time
             : (obj_type != VS.obj_type)
                 ? obj_type < VS.obj_type
...
struct HardSphereInfo
{
  ...
  HardSphereInfo() noexcept
    : coef_rest_n_obj(0.0),
      coef_rest_t_obj(0.0),
      coef_fri_obj(0.0),
      coef_rest_n_ptcl(0.0),
      coef_rest_t_ptcl(0.0),
      coef_fri_ptcl(0.0),
      coef_adh(0.0),
      coef_roll_fri(0.0)
  {
