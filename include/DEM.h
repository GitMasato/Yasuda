...
private:
  ...
  // input parameter
  Vec3 domain;                      // calculation_domain_length xyz (m)
  std::array<bool, 3> is_periodic;  // to apply periodic boundary xyz
  Vec3 coll_cell_ratio;  // cell size ratio to particle size, to sort particles
                         // spatially for collision calculation xyz
  Vec3 lr_cell_ratio;    // cell size ratio to particle size, to sort particles
                         // spatially for dipole interaction calculation xyz
  bool is_cell_z;        // to apply cells in z-direction
  double cell_domain_z;  // domain to be applied cells for collision or dipole
                         // calculations z (m)

  double time_step;        // time step  (s)
  double simulation_time;  // simulation time  (s)
  double output_time;      // time step to output the results  (s)
  bool is_ptcl_output;     // to output particle information in each interval
  ...
  bool is_avs_output;                 // to output MicroAVS file
  std::string avs_key;                // avs data keywords
  bool is_openmp;                     // to apply parallel computing by openmp
  ...
  bool is_airdrag;        // to apply air drag
  double viscosity;       // air viscosity (kg/s/m2)
  bool is_cunningham;     // to apply cunningham_correction
  double mean_free_path;  // mean free path (m)
  bool is_reynolds;       // to apply reynolds_number effect to air drag
  ...
  bool is_roll_fri;                         // to apply rolling friction
  double coef_roll_fri;                     // coefficient of rolling friction
  bool is_adh;                              // to apply adhesion force
  double coef_adh;                          // coefficient of adhesion force
  bool is_image_force;                      // to apply image force

  bool is_ele_field;          // to apply electrostatic field
  bool is_Coulomb_ext_field;  // to apply coulomb force from external field
  bool is_Coulomb_ptcl;       // to apply coulomb force from other particles
  bool is_diele_force;        // to apply dielectrophoresis_force
  bool is_diele_torque;       // to apply dielectrophoresis_torque
  bool is_diele_dipole;       // to apply dipole interaction
  double ele_update_time;     // time step to update electrostatic force  (s)
  std::array<double, 2> ele_start_stop_time;  // when to start and stop applying
                                              // electrostatic field (s)
  std::vector<std::array<double, 2>>
    ele_on_off_time;            // duration to turn-on and -off each field (s)
  std::vector<double> ele_amp;  // amplification rate to each field strength

  bool is_mag_field;       // to apply magnetic field
  bool is_mag_force;       // to apply magnetic force
  bool is_mag_torque;      // to apply magnetic torque
  bool is_mag_dipole;      // to apply magnetic dipole interactions
  double mag_update_time;  // time step to update magnetic force  (s)
  std::array<double, 2> mag_start_stop_time;  // when to start and stop applying
                                              // magnetic field (s)
  std::vector<std::array<double, 2>>
    mag_on_off_time;            // duration to turn-on and -off each field (s)
  std::vector<double> mag_amp;  // amplification rate to each field strength

  // temporal variable, coefficient
  int total_step;
  int intvl_output;
  int intvl_ele;
  int intvl_mag;
...
public:
  ...
  noexcept
    : domain(0.0, 0.0, 0.0),
      ...
      coll_cell_ratio(0.0, 0.0, 0.0),
      lr_cell_ratio(0.0, 0.0, 0.0),
      is_cell_z(false),
      cell_domain_z(0.0),
      time_step(0.0),
      simulation_time(0.0),
      output_time(0.0),
      is_ptcl_output(false),
      is_vtk_output(false),
      is_avs_output(false),
      is_openmp(false),
      openmp_size(0),
      ...
      coef_rest_obj{0.0, 0.0},
      coef_rest_ptcl{0.0, 0.0},
      fri_coef_obj_ptcl{0.0, 0.0},
      is_roll_fri(false),
      coef_roll_fri(0.0),
      is_adh(false),
      coef_adh(0.0),
      is_image_force(false),
      is_ele_field(false),
      is_Coulomb_ext_field(false),
      is_Coulomb_ptcl(false),
      is_diele_force(false),
      is_diele_torque(false),
      is_diele_dipole(false),
      ele_update_time(0.0),
      ele_start_stop_time{0.0, 0.0},
      is_mag_field(false),
      is_mag_force(false),
      is_mag_torque(false),
      is_mag_dipole(false),
      mag_update_time(0.0),
      mag_start_stop_time{0.0, 0.0},
      total_step(0),
      intvl_output(0),
      intvl_ele(0),
      intvl_mag(0),
  ...
  void SetFlagVariable(const std::vector<std::string>& Value,
                       const std::string& Key, bool& Frag,
  ...
  template <typename T>
  void SetArray(const std::vector<std::string>& Value, const std::string& Key,
                T& Parameter) noexcept
  {
    if (Value[0] == Key)
    {
  ...
  void SetUndefinedArray(const std::vector<std::string>& Value,
                         const std::string& Key, T& Parameter,
                         const std::size_t Check_Num) noexcept
  {
    if (Value[0] == Key)
    {
      const std::size_t values = Value.size();
