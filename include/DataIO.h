...
struct DataIO
{
  ...
  {
    std::cerr << "Error to read [" << Error_file << "]" << std::endl
              << std::endl;
    exit(1);
  }
  ...
  {
    std::cerr << "Error to output [" << Error_file << "]" << std::endl
              << std::endl;
    exit(1);
  }

  void SplitLine(const std::string& Line,
                 std::vector<std::string>& Split_Line) const noexcept
  {
    std::string v;
  ...
  std::list<std::string> Split(const std::string& Line,
                               const std::string& Delim) const noexcept
  {
    std::list<std::string> result;
    std::string Str = Line;
    std::size_t cutAt;

    while ((cutAt = static_cast<std::size_t>(Str.find_first_of(Delim))) !=
           Str.npos)
    {
      if (cutAt > 0)
      {
