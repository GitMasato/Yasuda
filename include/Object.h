...
public:
  ...
  std::vector<ObjectInfo>& GetFloorBase() noexcept { return floor_base; }

  std::vector<ObjectInfo>& GetFieldBase() noexcept { return field_base; }

  std::vector<ObjectInfo>& GetBoundaryBase() noexcept { return boundary_base; }

  std::vector<BoxInfo>& GetBox() noexcept { return box; }

  std::vector<HollowBoxInfo>& GetHollowBox() noexcept { return hollow_box; }

  std::vector<CylinderInfo>& GetCylinder() noexcept { return cylinder; }
  ...
  std::vector<SphereInfo>& GetSphere() noexcept { return sphere; }
  ...
  int GetBoxSize() const noexcept { return static_cast<int>(box.size()); }
  ...
  int GetSphereSize() const noexcept { return static_cast<int>(sphere.size()); }
  ...
  {
    const Vec3 pos(data_io.StringToDouble(P["pos_x"]),
    ...
    const Vec3 vec(data_io.StringToDouble(P["vib_x"]),
  ...
  template <typename T>
  void SetCollFlagVisualFlag(T& Obj,
                             std::map<std::string, std::string>& P) noexcept
  {
  ...
  {
    const Vec3 dim(data_io.StringToDouble(P["dim_x"]),
  ...
  template <typename T>
  void SetCylinderParameter(T& Obj,
                            std::map<std::string, std::string>& P) noexcept
  {
    Obj.SetLength(data_io.StringToDouble(P["length"]));

    const Vec3 dir(data_io.StringToDouble(P["dir_x"]),
    ...
    const Vec3 axis =
      cross_vector.LengthSq() != 0.0 ? cross_vector.Normalized() : unit_vector;
    Obj.SetRotAxis(axis);
  ...
  void ShowValue(const T& Obj) noexcept
  {
    ...
    std::cout << "Pos (" << pos.x << "," << pos.y << "," << pos.z << ")"
              << std::endl;
    std::cout << "Velo (" << velo.x << "," << velo.y << "," << velo.z << ")"
              << std::endl;
    ...
    std::cout << "Vib_Unit_Vec (" << vib_u_vec.x << "," << vib_u_vec.y << ","
              << vib_u_vec.z << ")" << std::endl;
  }
};
