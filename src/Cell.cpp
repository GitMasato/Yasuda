...
void Cell::ShowValue() const noexcept
{
  ...
  for (const auto& c : cell)
  {
    ...
    {
      std::cout << "tgt, related cell (" << c.id << "," << r.id << ","
                << r.offset << ")" << std::endl;
    }
  }
}
...
void Cell::CreateCell(const Vec3& Cell_Size_Ratio, const Vec3& Domain,
                      ...
                      const double Cell_Domain_Z) noexcept
{
  ...
  cell_size =
    (Domain / cell_length + 0.001).Int();  // 0.001: for rounding error
  ...
  {
    if (Cell_Size_Ratio.z < 1.0) cell_length.z = diameter_max;
    cell_size[2] = static_cast<int>(Cell_Domain_Z / cell_length.z +
                                    0.001);  // 0.001: for rounding error
    cell_origin.z = 0.0;
...
void Cell::SetPeriodicEffect(const Vec3& Domain,
                             const std::array<bool, 3>& is_Periodic) noexcept
{
...
void Cell::SetPeriodicEffect2DXY(const double DomainX,
                                 const double DomainY) noexcept
{
...
void Cell::SetPeriodicEffect2DXZ(const double DomainX,
                                 const double DomainZ) noexcept
{
  SetPeriodicEffect1DX(DomainX);
...
void Cell::SetPeriodicEffect2DYZ(const double DomainY,
                                 const double DomainZ) noexcept
{
  SetPeriodicEffect1DY(DomainY);
  SetPeriodicEffect1DZ(DomainZ);
...
void Cell::SetPeriodicEffect3D(const double DomainX, const double DomainY,
                               const double DomainZ) noexcept
{
  ...
  // opposing corner 1
  {
    ...
    const int id_1 =
      GetTgtMesh(cell_size[0] - 1, cell_size[1] - 1, cell_size[2] - 1);
    link_info[id_0].surr_cell.emplace_back(id_1, id_1_offset);
    link_info[id_1].surr_cell.emplace_back(id_0, id_0_offset);
  }
...
void Cell::SetLinkedSurrCell() noexcept
{
  ...
  for (int i = 0, c = GetCellSizeXYZ(); i < c; ++i)
  {
    for (int j = 0, lc = link_info[i].GetSurrCellSize(); j < lc; ++j)
    {
      ...
      const int link_surr_cell_size = copy_cell[link_cell_id].GetSurrCellSize();

      for (int k = 0; k < link_surr_cell_size; ++k)
      {
        const int link_surr_cell_id = copy_cell[link_cell_id].surr_cell[k].id;
        if (isDuplicate(i, link_surr_cell_id)) continue;

        cell[i].surr_cell.emplace_back(link_surr_cell_id,
  ...
  for (int i = 0, s = GetCellSizeXYZ(); i < s; ++i)
  {
    ...
    for (int j = 0, sc = copy_cell[i].GetSurrCellSize(); j < sc; ++j)
    {
      ...
      const int surr_link_cell_size = link_info[surr_cell_id].GetSurrCellSize();

      for (int k = 0; k < surr_link_cell_size; ++k)
      {
        const int surr_link_cell_id = link_info[surr_cell_id].surr_cell[k].id;

        if (isDuplicate(i, surr_link_cell_id)) continue;

        cell[i].surr_cell.emplace_back(
          surr_link_cell_id, copy_cell[i].surr_cell[j].offset +
  ...
  for (int i = 0, s = GetCellSizeXYZ(); i < s; ++i)
  {
    for (int j = 0, lc = link_info[i].GetSurrCellSize(); j < lc; ++j)
    {
      ...
      cell[i].surr_cell.emplace_back(link_cell_id,
                                     link_info[i].surr_cell[j].offset);
    }
  }
  ...
  for (int i = 0, s = GetCellSizeXYZ(); i < s; ++i)
  {
    ...
    for (int j = 0, sc = copy_cell[i].GetSurrCellSize(); j < sc; ++j)
    {
      ...
      for (int k = 0; k < surr_link_cell_size; ++k)
      {
        ...
        cell[i].surr_cell.emplace_back(
          surr_link_cell_id, link_info[surr_cell_id].surr_cell[k].offset);
      }
    }
  }
...
void Cell::SetPtclInCell(const bool is_Periodic_Z, const bool is_Cell_Z,
                         const double Cell_Domain_Z) noexcept
{
  if (is_Periodic_Z)
  {
...
#pragma omp for schedule(dynamic)
  for (int i = 0; i < ptcl_stay_size; ++i)
  {
    ...
    const int id = (rel_pos.z < 0.0) ? X * tgt[1] + tgt[0]
                                     : (Cell_Domain_Z < rel_pos.z)
                                         ? XY_Zminus + X * tgt[1] + tgt[0]
...
void Cell::GetNeighbPtclList(const int ID,
                             std::vector<NeighbPtclInfo>& Ptcl_List) noexcept
{
  Ptcl_List.clear();
