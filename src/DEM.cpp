...
#pragma omp single
    {
      ...
      if (is_avs_output)
        avs.CreateAVS(avs_step, gravity, domain, avs_file, avs_key);
...
#pragma omp single
      ...
      if (!is_image_force)
        coll.DetectCollObj(is_openmp, openmp_size);
      else
        coll.DetectCollObjImage(is_openmp, openmp_size, coef_image);

// calculate collision
#pragma omp single
      {
        coll.SortCollInfo();
        if (is_roll_fri && is_adh)
          coll.CalcCollAdhRoll(hard_sphere_info);
        else if (is_roll_fri)
          coll.CalcCollRoll(hard_sphere_info);
        else if (is_adh)
          coll.CalcCollAdh(hard_sphere_info);
        else
          coll.CalcColl(hard_sphere_info);
      }
...
#pragma omp single
        {
          ...
          if (is_vtk_output) vtk.CreateFile(time, vtk_file, vtk_key);
          if (is_avs_output)
            avs.WriteAVS(time, gravity, domain, avs_file, avs_key);
          output_step += intvl_output;
        }
      }
...
void DEM::CalcEleForce() noexcept
{
  if (ele_field.isExtFieldOn())
  {
    ...
    if (is_diele_dipole)
    {
      ...
      if (!is_Coulomb_ptcl)
        lr_ptcl_list.SetPtclInfo();
      else
        lr_ptcl_list.SetPtclInfoCalcCoulombPtcl(coef_ele);
...
void DEM::SumForceTorque() noexcept
{
  ...
  if (is_cunningham)
    ptcl.AddAirDragCunningham(air_drag_info);
  else if (is_reynolds)
    ptcl.AddAirDragReynolds(air_drag_info);
  else
    ptcl.AddAirDrag(air_drag_info);
...
void DEM::CreateCell() noexcept
{
  ...
  cell.CreateCell(coll_cell_ratio, domain, is_periodic[2], is_cell_z,
                  cell_domain_z);
  cell.SetCellInfo();
  cell.SetPeriodicEffect(domain, is_periodic);
  std::cout << "linking cell..." << std::endl;
  cell.SetLinkedSurrCell();
  std::cout << "finish creating virtual cell, for collision detection"
            << std::endl;
  std::cout << std::endl;

  if (is_Coulomb_ptcl || is_diele_dipole || is_mag_dipole)
  {
    std::cout << "creating virtual cell, for long range interaction..."
              << std::endl;
    lr_cell.CreateCell(lr_cell_ratio, domain, is_periodic[2], is_cell_z,
                       cell_domain_z);
    lr_cell.SetCellInfo();
    lr_cell.SetPeriodicEffect(domain, is_periodic);
    std::cout << "linking cell..." << std::endl;
    lr_cell.SetLinkedSurrCell();
    std::cout << "finish creating virtual cell, for long range interaction"
              << std::endl;
    std::cout << std::endl;
  }
...
void DEM::SetPathInput(const int& argc, char** argv) noexcept
{
  ...
  for (int i = 1; i < argc; ++i)
  {
    ...
    {
      const fs::path p =
        (input.is_absolute()) ? input : fs::current_path() / input;
...
void DEM::SetPathOutput() noexcept
{
  ...
  fs::path path_ini = ini_input_file;
  std::string ini_filename = path_ini.filename().string();
  std::string ini_filename_without_extension =
    ini_filename.substr(0, ini_filename.length() - 7);

  fs::path path_output_dir = path_ini.parent_path();
  path_output_dir.append(ini_filename_without_extension);
  fs::create_directory(path_output_dir);

  fs::path path_ini_copy = path_output_dir;
  path_ini_copy.append(ini_filename);
  if (fs::exists(path_ini_copy)) fs::remove(path_ini_copy);
  fs::copy(ini_input_file, path_ini_copy.string());
  std::cout << "copy [" << path_ini << "]" << std::endl;

  fs::path path_output_1 = path_output_dir;
  path_output_1.append("OUTPUT_1");
  if (fs::exists(path_output_1)) fs::remove_all(path_output_1);
  fs::create_directory(path_output_1);
  std::cout << "create [" << path_output_1 << "] directory" << std::endl;

  fs::path path_ptcl_stay_file = path_output_1;
  path_ptcl_stay_file.append(ini_filename_without_extension + "_stay_ptcl.csv");
  ptcl_stay_last_file = path_ptcl_stay_file.string();

  fs::path path_ptcl_out_file = path_output_1;
  path_ptcl_out_file.append(ini_filename_without_extension + "_out_ptcl.csv");
  ptcl_out_last_file = path_ptcl_out_file.string();

  fs::path path_time_file = path_output_1;
  path_time_file.append(ini_filename_without_extension + "_time.txt");
  time_file = path_time_file.string();
  ...
  if (is_avs_output)
  {
    fs::path path_avs = path_output_1;
    path_avs.append(ini_filename_without_extension + "_ptcl.mgf");
    avs_file = path_avs.string();
    scope.ShowVariable(avs_file, "avs_file");
  }

  if (is_ptcl_output)
  {
    fs::path path_output_2 = path_output_dir;
    path_output_2.append("OUTPUT_2");
    if (fs::exists(path_output_2)) fs::remove_all(path_output_2);
    fs::create_directory(path_output_2);
    std::cout << "create [" << path_output_2 << "] directory" << std::endl;

    fs::path path_ptcl_interval_file = path_output_2;
    path_ptcl_interval_file.append("_");
    ptcl_file = path_ptcl_interval_file.string();
    scope.ShowVariable(ptcl_file, "ptcl_interval_file");
  }

  if (is_vtk_output)
  {
    fs::path path_output_3 = path_output_dir;
    path_output_3.append("OUTPUT_3");
    if (fs::exists(path_output_3)) fs::remove_all(path_output_3);
    fs::create_directory(path_output_3);
    std::cout << "create [" << path_output_3 << "] directory" << std::endl;

    fs::path path_vtk = path_output_3;
    path_vtk.append("_");
    vtk_file = path_vtk.string();
...
void DEM::LoadParamater() noexcept
{
  ...
  while (!fin.eof() && getline(fin, line))
  {
    ...
    SetFlagUndefinedArray(key, "Enable_Particle_Info_Output", is_ptcl_output,
                          ptcl_key);
    ...
    SetFlagVariable(key, "Enable_Cunningham_Correction", is_cunningham,
                    mean_free_path);
    SetFlagVariable(key, "Enable_Reynolds_Correction", is_reynolds,
                    density_air);
    SetArray(key, "Permittivity_Particle_Air_Object",
             permittivity_ptcl_air_obj);
    ...
    if (is_ele_field)
    {
      ...
      SetUndefinedPairArray(key, "Electrostatic_Field_On_Off", ele_on_off_time,
                            ele_input_file.size());
      SetUndefinedArray(key, "Electrostatic_Field_Amplification", ele_amp,
                        ele_input_file.size());
    }
    ...
    if (is_mag_field)
    {
      ...
      SetUndefinedPairArray(key, "Magnetic_Field_On_Off", mag_on_off_time,
                            mag_input_file.size());
      SetUndefinedArray(key, "Magnetic_Field_Amplification", mag_amp,
                        mag_input_file.size());
    }
  }
  ...
  ShowValue();
  std::cout << "finish loading ini [" << ini_input_file << "] file"
            << std::endl;
  fin.close();
}
