...
void Field::ShowValue() noexcept
{
  for (const auto& n : field)
  {
    ...
    std::cout << "mesh_size (" << m[0] << "," << m[1] << "," << m[2] << ")"
              << std::endl;
                      ...
                      const std::vector<double>& Amp) noexcept
{
  ...
  for (int i = 0; i < total_file_size; ++i)
  {
    ...
    if ((field[i].GetMeshSizeXY() <= 1) || (field[i].GetMeshSizeXZ() <= 1) ||
  ...
  {
          ofstream ofs;
          ofs.open(Field_Filename[i], ios_base::out | ios_base::binary |
  ios_base::trunc);
          ...
          std::array<int, 3> mesh_size;
          mesh_size[0] = static_cast<int>(domain.x * mesh_length_inv + 1e-9) +
  1; mesh_size[1] = static_cast<int>(domain.y * mesh_length_inv + 1e-9) + 1;
          mesh_size[2] = static_cast<int>(domain.z * mesh_length_inv + 1e-9) +
  1;

          const int total_mesh_size = mesh_size[0] * mesh_size[0] *
  mesh_size[0]; vector<Vec3> field(total_mesh_size, Vec3::UnitX());
...
void Field::SetPtclFieldInfo(const std::array<bool, 3>& is_Periodic) noexcept
{
  ...
  if (mesh_size[2] <= 1)
    SetPtclFieldInfo2DXY(is_Periodic);
  else if (mesh_size[1] <= 1)
    SetPtclFieldInfo2DXZ(is_Periodic);
  else if (mesh_size[0] <= 1)
    SetPtclFieldInfo2DYZ(is_Periodic);
  else
    SetPtclFieldInfo3D(is_Periodic);
}

void Field::SetPtclFieldInfo2DXY(
...
#pragma omp for schedule(dynamic)
    for (int i = 0; i < ptcl_stay_size; ++i)
    {
      ...
      const Vec3 right =
        field[tgt_field].GetXPlusFieldPeriodic(int_pos[0], tgt);
      const Vec3 front =
        field[tgt_field].GetYMinusFieldXYPeriodic(int_pos[1], tgt);
      const Vec3 back =
        field[tgt_field].GetYPlusFieldXYPeriodic(int_pos[1], tgt);
...
void Field::SetPtclFieldInfo2DXZ(
  const std::array<bool, 3>& is_Periodic) noexcept
{
  std::vector<ObjectInfo>& o = obj->GetFieldBase();
...
#pragma omp for schedule(dynamic)
    for (int i = 0; i < ptcl_stay_size; ++i)
    {
      ...
      const Vec3 right =
        field[tgt_field].GetXPlusFieldPeriodic(int_pos[0], tgt);
      const Vec3 down =
        field[tgt_field].GetZMinusFieldXZPeriodic(int_pos[1], tgt);
...
void Field::SetPtclFieldInfo2DYZ(
  const std::array<bool, 3>& is_Periodic) noexcept
{
  std::vector<ObjectInfo>& o = obj->GetFieldBase();
...
#pragma omp for schedule(dynamic)
    for (int i = 0; i < ptcl_stay_size; ++i)
    {
      ...
      const Vec3 front =
        field[tgt_field].GetYMinusFieldYZPeriodic(int_pos[0], tgt);
      const Vec3 back =
        field[tgt_field].GetYPlusFieldYZPeriodic(int_pos[0], tgt);
      const Vec3 down =
        field[tgt_field].GetZMinusFieldYZPeriodic(int_pos[1], tgt);
...
#pragma omp for schedule(dynamic)
    for (int i = 0; i < ptcl_stay_size; ++i)
    {
      ...
      const Vec3 back =
        field[tgt_field].GetYPlusFieldXYPeriodic(int_pos[1], tgt);
      const Vec3 down =
        field[tgt_field].GetZMinusField3DPeriodic(int_pos[2], tgt);
      const Vec3 up = field[tgt_field].GetZPlusField3DPeriodic(int_pos[2], tgt);
      const auto fd =
        GetFieldDeriv3D(m_double_inv, left, right, front, back, down, up);
      field_at_ptcl[i].SetExtField(field[tgt_field].GetField(tgt), fd);
    }
  }
