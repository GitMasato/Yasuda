...
namespace dem
{
	...
	public:
		...
		LongRange() noexcept
			: ptcl(nullptr), obj(nullptr), field(nullptr), lr_ptcl_list(nullptr)
			, sum_energy(0.0), pre_sum_energy(0.0) {}
		...
		void AssociateField(Field* Field) noexcept 
		{
			field = Field;
		}
		...
		void CalcEleDipoleInteract(const double Dipole_Coef, const double Field_Coef) noexcept;

		void CalcMagDipoleInteract(const double Dipole_Coef, const double Field_Coef) noexcept;
