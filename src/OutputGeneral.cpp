...
namespace dem
{
	...
	void OutputGeneral::CreateTimeFile(const int Total_Time_Step,
		const std::array<double, 2> Energy, const std::string& File) const noexcept
	{
		std::ofstream fout;
		fout.open(File, std::ios_base::out | std::ios_base::trunc);
		if (!fout) data_io.ErrorOutput(File);
		
		fout << "DEM START" << std::endl;
		std::cout << "DEM START" << std::endl;
		fout.close();
	...
	void OutputGeneral::WriteTimeFile(const int Time_Step, const int Total_Time_Step,
		const std::array<double, 2> Energy, const std::string& File) const noexcept
	{
		std::ofstream fout;
		...
		fout << " step (" << Time_Step << "/" << Total_Time_Step << ")"
			<< "  erapsed_time (" << profiler.Print() / 3600 << ":"
			<< (profiler.Print() % 3600) / 60 << ":" << (profiler.Print() % 3600) % 60 << ")"
			<< "  energy[" << ptcl_stay_size << "] (" << Energy[0] << "," << Energy[1] << ")" 
			<< std::endl;
	...
	void OutputGeneral::WriteTimeFileEnd(const std::string& File) const noexcept
	{
		std::ofstream fout;
		fout.open(File, std::ios_base::out | std::ios_base::app);
		if (!fout) data_io.ErrorOutput(File);
	...
	void OutputGeneral::CreatePtclStayFile(const std::string& filename) const noexcept
	{
		std::ofstream fout;
		...
		fout << ptcl->GetPtclStaySize() << "," << "number" << std::endl;
	...
	void OutputGeneral::CreatePtclOutFile(const std::string& filename) const noexcept
	{
		std::ofstream fout;
		fout.open(filename, std::ios_base::out | std::ios_base::trunc);
		if (!fout)	data_io.ErrorOutput(filename);
		...
		fout << ptcl->GetPtclOutSize() << "," << "number" << std::endl;
		fout << "x(m),y(m),z(m),u(m/s),v(m/s),w(m/s),"
			<< "omega1(rad/s),omega2(rad/s),omega3(rad/s),"
			<< "sp_gravity(kg/m3),diameter(m),charge(C)" << std::endl;
	...
	void OutputGeneral::CreatePtclFile(const double Elapsed_Time,
		const std::string& File, std::vector<std::string> Keys) const noexcept
	{
		std::stringstream ss;
		ss << File << std::setw(5) << std::setfill('0')	<< int((Elapsed_Time * 1000) + 0.5);
		ss << "_ms.dem";
		std::string file = ss.str();

		std::ofstream fout;
		fout.open(file, std::ios_base::out | std::ios_base::binary | std::ios_base::trunc);
		if (!fout) data_io.ErrorOutput(file);
	...
	void OutputGeneral::WriteID(const char Key[3], std::ofstream& Fout,
	...
	void OutputGeneral::WriteCollNumPtcl(const char Key[3], std::ofstream& Fout,
		std::vector<ParticleInfo>& P) const noexcept
	{
	...
	void OutputGeneral::WriteCollNumObj(const char Key[3], std::ofstream& Fout,
		std::vector<ParticleInfo>& P) const noexcept
	{
	...
	void OutputGeneral::WriteRadius(const char Key[3], std::ofstream& Fout,
		std::vector<ParticleInfo>& P) const noexcept
	{
	...
	void OutputGeneral::WriteMass(const char Key[3], std::ofstream& Fout,
		std::vector<ParticleInfo>& P) const noexcept
	{
	...
	void OutputGeneral::WriteUnitCharge(const char Key[3], std::ofstream& Fout,
		std::vector<ParticleInfo>& P) const noexcept
	{
	...
	void OutputGeneral::WriteCharge(const char Key[3], std::ofstream& Fout,
		std::vector<ParticleInfo>& P) const noexcept
	{
	...
	void OutputGeneral::WriteEleDipoleEnergy(const char Key[3], std::ofstream& Fout) const noexcept
	{
	...
	void OutputGeneral::WriteMagDipoleEnergy(const char Key[3], std::ofstream& Fout) const noexcept
	{
	...
	void OutputGeneral::WritePos(const char Key[3], std::ofstream& Fout,
		std::vector<ParticleInfo>& P) const noexcept
	{
	...
	void OutputGeneral::WriteVelo(const char Key[3], std::ofstream& Fout,
		std::vector<ParticleInfo>& P) const noexcept
	{
	...
	void OutputGeneral::WriteAngVelo(const char Key[3], std::ofstream& Fout,
		std::vector<ParticleInfo>& P) const noexcept
	{
	...
	void OutputGeneral::WriteAng(const char Key[3], std::ofstream& Fout,
		std::vector<ParticleInfo>& P) const noexcept
	{
	...
	void OutputGeneral::WriteForce(const char Key[3], std::ofstream& Fout,
		std::vector<ParticleInfo>& P) const noexcept
	{
	...
	void OutputGeneral::WriteTorque(const char Key[3], std::ofstream& Fout,
		std::vector<ParticleInfo>& P) const noexcept
	{
	...
	void OutputGeneral::WriteCoulombExtForce(const char Key[3], std::ofstream& Fout,
		std::vector<ParticleInfo>& P) const noexcept
	{
	...
	void OutputGeneral::WriteCoulombPtclForce(const char Key[3], std::ofstream& Fout,
		std::vector<ParticleInfo>& P) const noexcept
	{
	...
	void OutputGeneral::WriteImageForce(const char Key[3], std::ofstream& Fout,
		std::vector<ParticleInfo>& P) const noexcept
	{
	...
	void OutputGeneral::WriteDieleForce(const char Key[3], std::ofstream& Fout,
		std::vector<ParticleInfo>& P) const noexcept
	{
	...
	void OutputGeneral::WriteMagForce(const char Key[3], std::ofstream& Fout,
		std::vector<ParticleInfo>& P) const noexcept
	{
	...
	void OutputGeneral::WriteAdhForce(const char Key[3], std::ofstream& Fout,
		std::vector<ParticleInfo>& P) const noexcept
	{
	...
	void OutputGeneral::WriteDieleTorque(const char Key[3], std::ofstream& Fout,
		std::vector<ParticleInfo>& P) const noexcept
	{
	...
	void OutputGeneral::WriteMagTorque(const char Key[3], std::ofstream& Fout,
		std::vector<ParticleInfo>& P) const noexcept
	{
