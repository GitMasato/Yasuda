...
void OutputVTK::WritePos(std::ofstream& Fout,
...
void OutputVTK::WriteID(std::ofstream& Fout, std::vector<ParticleInfo>& P) const
  noexcept
{
                                 ...
                                 std::vector<ParticleInfo>& P) const noexcept
{
  ...
  for (const auto& i : P)
  {
    Fout << i.GetCollNumPtcl() << std::endl;
  }
}
