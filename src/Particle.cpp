...
namespace dem
{
	...
	void Particle::AddAirDrag(AirDragInfo& Info) noexcept
	{
		...
		for (int i = 0; i < ptcl_stay_size; ++i)
		{
			...
			const Vec3 air_drag = -air_coef * (ptcl_stay[i].GetHStepVelo() - Info.air_velo);
	...
	{
		if ((Info.coef_allen > 0.0) && (Info.coef_newton > 0.0) && (Info.coef_stokes > 0.0))
		{	
			const int ptcl_stay_size = GetPtclStaySize();
	...
	void Particle::TimeIntegVeloVerlet(const double Time_Step) noexcept
	{
		...
		for (int i = 0; i < ptcl_stay_size; ++i)
		{
			const Vec3 post_accel = ptcl_stay[i].GetPostExtForce() * ptcl_stay[i].GetMassInv();
	...
	void Particle::TimeIntegHAngVeloAngVerlet(const double Time_Step) noexcept
	{
		...
		for (int i = 0; i < ptcl_stay_size; ++i)
		{
			const Vec3 ang_accel = ptcl_stay[i].GetExtTorque() * ptcl_stay[i].GetInertialMomentInv();
	...
	void Particle::TimeIntegAngVeloVerlet(const double Time_Step) noexcept
	{
		...
		for (int i = 0; i < ptcl_stay_size; ++i)
		{
			...
			const Vec3 post_ang_accel = post_ext_torque * ptcl_stay[i].GetInertialMomentInv();
