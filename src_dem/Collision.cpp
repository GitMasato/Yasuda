...
namespace dem
{
	...
	void Collision::ClearCollOrder() noexcept
	{
		coll_info.clear();
	}
	...
	void Collision::DetectPtclCollPeriodicX(const Vec3& Domain, const int Reserve) noexcept
	{
		std::vector<CollisionInfo> coll;
		coll.reserve(Reserve);
	...
	void Collision::DetectPtclCollPeriodicY(const Vec3& Domain, const int Reserve) noexcept
	{
		std::vector<CollisionInfo> coll;
		coll.reserve(Reserve);
	...
	void Collision::DetectPtclCollPeriodicZ(const Vec3& Domain, const int Reserve) noexcept
	{
		std::vector<CollisionInfo> coll;
		coll.reserve(Reserve);
		...
		for (int i = 0; i < ptcl_stay_size; ++i)
		{
			...
			for (const auto& v : potential_coll_list)
			{
				...
				const double dist_sq_diff = r_pos.LengthSq() - R_sum_sq;
				const auto [overlap, u_n_vec] = GetOverlapVec(post_dist_sq, R_sum, r_post_pos);
	...
	void Collision::DetectPtclCollPeriodicXY(const Vec3& Domain, const int Reserve) noexcept
	{
		std::vector<CollisionInfo> coll;
		coll.reserve(Reserve);
	...
	void Collision::DetectPtclCollPeriodicXZ(const Vec3& Domain, const int Reserve) noexcept
	{
		std::vector<CollisionInfo> coll;
		coll.reserve(Reserve);
	...
	void Collision::DetectPtclCollPeriodicYZ(const Vec3& Domain, const int Reserve) noexcept
	{
		std::vector<CollisionInfo> coll;
		coll.reserve(Reserve);
	...
	void Collision::DetectPtclCollPeriodicXYZ(const Vec3& Domain, const int Reserve) noexcept
	{
		std::vector<CollisionInfo> coll;
		coll.reserve(Reserve);
	...
	void Collision::DetectCollObj(const bool is_Openmp, const int Openmp_Size) noexcept
	{
		const int ptcl_stay_size = ptcl->GetPtclStaySize();
		const int process_size = is_Openmp ? Openmp_Size : 1;
	...
	void Collision::DetectFloorCollImage(const double Image_Coef, const int Reserve) noexcept
	{
		if (!(obj->GetFloorBaseSize()))	return;
		...
		for (int i = 0; i < ptcl_stay_size; ++i)
		{
			...
			{
				p[i].AddImageForceZ(-GetImageForceLen(r_post_pos, Image_Coef, p[i].GetChgSq()));
				continue;
			}
	...
	void Collision::DetectBoxColl(const int Reserve) noexcept
	{
		...
		for (int k = 0; k < box_size; ++k)
		{
			...
			for (int i = 0; i < ptcl_stay_size; ++i)
			{
				...
				const bool is_in_box = ClampPos(o_post_coll_pt, o_post_pos_min, o_post_pos_max);
				
				// detect collision
				if (is_in_box)
				{
					p[i].AddCollNumObj();
					const auto [dir, post_dist] = GetCloseDirDist(p_post_pos, o_post_pos_min, o_post_pos_max);
					const double p_R = p[i].GetRadius();
					...
					else
					{
						...
						const double r_h_velo = GetDiffCloseDir(dir, p_h_velo, o_h_velo, p_h_velo);
						const double time = surf_dist / r_h_velo;
				...
				else
				{
					...
					p[i].AddCollNumObj();
					const auto [overlap, u_n_vec] = GetOverlapVec(post_dist_sq, p_R, r_post_pos);
	...
	void Collision::DetectBoxCollImage(const double Image_Coef, const int Reserve) noexcept
	{
		std::vector<CollisionInfo> coll;
		coll.reserve(Reserve);
		...
		for (int k = 0; k < box_size; ++k)
		{
			...
			for (int i = 0; i < ptcl_stay_size; ++i)
			{
				...
				if (is_in_box)
				{
					...
					{
						const Vec3 p_h_velo = p[i].GetHStepVelo();
						const double r_h_velo = GetDiffCloseDir(dir, p_h_velo, o_h_velo, o_h_velo);
						const double time = surf_dist / r_h_velo;
						coll.emplace_back(2, i, k, time, overlap, u_n_vec, o_h_velo);
					}
				}
			...
			for (int i = 0; i < ptcl_stay_size; ++i)
			{
				...
				if (is_in_box)
				{
					...
					else if (post_dist <= o_thk + p_R)
					{
						...
						const Vec3 u_n_vec = -GetUNVecCloseDir(dir);
						const Vec3 p_pos = p[i].GetPos();
						const double dist = GetDiffCloseDir(dir, p_pos, o_pos_min_in, o_pos_max_in);
						const double surf_dist = dist - p_R;
	...
	void Collision::DetectHollowBoxCollImage(
		const double Image_Coef, const int Reserve) noexcept
	{		
		std::vector<CollisionInfo> coll;
		coll.reserve(Reserve);
	...
	{			
		...
		for (int k = 0; k < cylinder_size; ++k)
		{
			...
			for (int i = 0; i < ptcl_stay_size; ++i)
			{
				...
				else if ((o_post_pos_rot.x <= p_post_pos_rot.x) && (p_post_pos_rot.x <= o_post_pos_rot.x + o_len))
				{
					...
					else
					{
						...
						const double time = GetCollTime(r_pos_rot, r_h_velo_rot, dist_sq_diff);
				...
				{
					r_post_pos_rot.x = 0.0;
					Vec3 pos_corr_rot = -(o_outer_R) * (r_post_pos_rot / sqrt(post_dist_yz_rot_sq));
					if (o_post_pos_rot_x_mid <= p_post_pos_rot.x) pos_corr_rot.x += o_len;

					const Vec3 r_post_pos_coll_pt_rot = o_post_pos_rot + pos_corr_rot - p_post_pos_rot;
	...
	void Collision::DetectCylinderCollImage(const double Image_Coef, const int Reserve) noexcept
	{		
		std::vector<CollisionInfo> coll;
		coll.reserve(Reserve);
	...
	{			
		...
		for (int k = 0; k < hollow_cylinder_size; ++k)
		{
			...
			const double o_outer_R = o[k].GetOuterRadius();
			const double o_middle_R_sq = (o_inner_R + o_outer_R) * (o_inner_R + o_outer_R) * 0.25;
			...
			for (int i = 0; i < ptcl_stay_size; ++i)
			{
				...
				if ((o_inner_R_sq <= post_dist_yz_rot_sq) && (post_dist_yz_rot_sq <= o_outer_R_sq))
				{
					if (p_post_pos_rot.x <= o_post_pos_rot_x_mid)
					{
						// no collision
				...
				{
					r_post_pos_rot.x = 0.0;
					Vec3 pos_corr_rot = post_dist_yz_rot_sq <= o_middle_R_sq
					...
					const Vec3 r_pos_rot = o_pos_rot + pos_corr_rot - o[k].Rotated(p[i].GetPos());
					const double dist_sq_diff = r_pos_rot.LengthSq() - p_R_sq;
	...
	void Collision::DetectHollowCylinderCollImage(const double Image_Coef, const int Reserve) noexcept
	{
		std::vector<CollisionInfo> coll;
		coll.reserve(Reserve);
	...
	void Collision::DetectSphereCollImage(const double Image_Coef, const int Reserve) noexcept
	{
		std::vector<CollisionInfo> coll;
		coll.reserve(Reserve);
	...
	void Collision::DetectHollowSphereCollImage(const double Image_Coef, const int Reserve) noexcept
	{	
		std::vector<CollisionInfo> coll;
		coll.reserve(Reserve);
	...
	void Collision::CalcColl(const HardSphereInfo& C) noexcept
	{
		...
		for (int i = 0; i < coll_info_size; ++i)
		{
			if (coll_info[i].obj_type != 0)
			{
				...
				const auto [Jn_len, u_t_vec] = GetJnLen(pt_velo, u_n_vec, C.coef_rest_n_obj, m_A);
			...
			else
			{
				...
				const auto [Jn_len, u_t_vec] = GetJnLen(pt_velo, u_n_vec, C.coef_rest_n_ptcl, m_inv_AB_inv);
				const double Jt_len = GetJtLen(pt_velo, u_t_vec, C.coef_rest_t_ptcl, m_inv_AB_inv);
