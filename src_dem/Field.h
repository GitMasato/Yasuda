...
namespace dem
{
	...
	public:
		...
		Field() noexcept : ptcl(nullptr), obj(nullptr), is_ext_field_on(false), tgt_field(-1) {}
		...
		bool isExtFieldOn() const noexcept
		{
			return is_ext_field_on;
		}

		int GetFieldSize() const noexcept 
		{
		...
		void SetTgtField(const double Time, const std::array<double, 2>& Start_Stop_Time,
		...
		std::array<Vec3, 3> GetFieldDeriv2DXY(const double Mesh_Double_Inv,
			const Vec3& Left, const Vec3& Right, const Vec3& Front, const Vec3& Back) const noexcept
		{
		...
		std::array<Vec3, 3> GetFieldDeriv2DXZ(const double Mesh_Double_Inv,
			const Vec3& Left, const Vec3& Right, const Vec3& Down, const Vec3& Up) const noexcept
		{
			const Vec3 diff_x = (Right - Left) * Mesh_Double_Inv;
		...
		std::array<Vec3, 3> GetFieldDeriv2DYZ(const double Mesh_Double_Inv,
			const Vec3& Front, const Vec3& Back, const Vec3& Down, const Vec3& Up) const noexcept
		{
			const Vec3 diff_y = (Back - Front) * Mesh_Double_Inv;
			const Vec3 diff_z = (Up - Down) * Mesh_Double_Inv;
		...
		std::array<Vec3, 3> GetFieldDeriv3D(const double Mesh_Double_Inv,
			...
			const Vec3& Down, const Vec3& Up) const noexcept
		{
			const Vec3 diff_x = (Right - Left) * Mesh_Double_Inv;
			const Vec3 diff_y = (Back - Front) * Mesh_Double_Inv;
			const Vec3 diff_z = (Up - Down) * Mesh_Double_Inv;
