...
namespace dem
{
	...
	public:
		...
		constexpr void SetExtField(const Vec3& F, const std::array<Vec3, 3>& Fd) noexcept
		{
			field = F;
			field_deriv = Fd;
		}
		...
		void SetFieldAsTotalField() noexcept
		{
			field = GetFieldTotal();
		}


		void SetMoment(const Vec3& F) noexcept
		{
			moment = F;
		}


		void SetFieldPtcl(const Vec3& F) noexcept
		{
			field_ptcl = F;
		}


		Vec3 GetField() const noexcept
		{
			return field;
		}


		Vec3 GetFieldTotal() const noexcept
		{
			return field + field_ptcl;
		}


		Vec3 GetMoment() const noexcept
		{
			return moment;
		}


		double GetMomentEnergy() const noexcept
		{
			return field.Dot(moment);
		}


		std::array<Vec3, 3> GetFieldDeriv() const noexcept
		{
			return field_deriv;
		}
	...
	public:
		...
		std::array<int, 3> GetMeshSize() const noexcept
		{
			return mesh_size;
		}


		double GetMeshLength() const noexcept
		{
			return mesh_len;
		}


		double GetMeshLengthInv() const noexcept
		{
			return mesh_len_inv; 
		}
							

		int GetMeshSizeXY() const noexcept 
		{
			return mesh_size[0] * mesh_size[1];
		}
		

		int GetMeshSizeXZ() const noexcept 
		{
			return mesh_size[0] * mesh_size[2];
		}


		int GetMeshSizeYZ() const noexcept 
		{
		...
		void SetMeshLength(const double v) noexcept
		{ 
			mesh_len = v; 
		}


		void SetMeshLengthInv(const double v) noexcept
		{ 
			mesh_len_inv = v;
		}
		

		void SetMeshSize(const std::array<int, 3>& v) noexcept
		{
			mesh_size = v;
		}
		

		void MoveSetField(std::vector<Vec3>& v) noexcept 
		{
			field = std::move(v); 
		}
		...
		std::array<int, 2> GetIntPos(const Vec2& Rel_Pos) const noexcept
		{
			...
			return 
			{
				static_cast<int>(pos_mesh.x), 
				static_cast<int>(pos_mesh.y) 
			};
		}
		...
		Vec3 GetField(const int Tgt) const noexcept
		{
			return field[Tgt];
		}
		...
		{
			return ((0 < XY[0]) && (XY[0] < mesh_size[0]) 
				&& (0 < XY[1]) && (XY[1] < mesh_size[1]))
				? true : false;
		}
		...
		{
			return ((0 < XZ[0]) && (XZ[0] < mesh_size[0]) 
				&& (0 < XZ[1]) && (XZ[1] < mesh_size[2]))
				? true : false;
		}
		...
		{
			return ((0 < YZ[0]) && (YZ[0] < mesh_size[1]) 
				&& (0 < YZ[1]) && (YZ[1] < mesh_size[2]))
				? true : false;
		}
		...
		{
			return ((0 < XYZ[0]) && (XYZ[0] < mesh_size[0])
				&& (0 < XYZ[1]) && (XYZ[1] < mesh_size[1])
				&& (0 < XYZ[2]) && (XYZ[2] < mesh_size[2]))
				? true : false;
		}
