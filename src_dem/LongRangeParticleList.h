...
namespace dem
{
	...
	struct LongRangePtcl
	{
		...
		LongRangePtcl(const int ID, 
			const double D3, const double D5, const double D7, 
			const Vec3& RP) noexcept
			: id(ID)
			, dist_inv_3(D3), dist_inv_5(D5), dist_inv_7(D7)
			, rel_pos(RP) {}
	};
	...
	public:
		...
		void AssociateLRCell(Cell* LR_Cell) noexcept
		{
			lr_cell = LR_Cell;
		}

		bool isLRPtclListUpdated() const noexcept
		{
			return is_lr_ptcl_list_updated;
		}
		...
		void LRPtclListisNotUpdated() noexcept
		{
			is_lr_ptcl_list_updated = false;
		}
