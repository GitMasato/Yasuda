...
namespace dem
{
	...
	public:
		...
		Particle() noexcept : obj(nullptr), is_ptcl_stay_size_changed(false),
			min_diameter(0.0), max_diameter(0.0), trans_energy(0.0), rot_energy(0.0) {}

		std::vector<ParticleInfo>& GetPtclStay() noexcept
		{
			return ptcl_stay; 
		}

		std::vector<ParticleInfo>& GetPtclOut() noexcept
		{
			return ptcl_out;
		}
		...
		double GetMinDiameter() const noexcept
		{
			return min_diameter;
		}

		double GetMaxDiameter() const noexcept
		{
			return max_diameter;
		}
		...
		void SetMinDiameter(const double v) noexcept
		{
			min_diameter = v;
		}

		void SetMaxDiameter(const double v) noexcept
		{
			max_diameter = v;
		}
