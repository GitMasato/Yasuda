...
Magnetic_Calc::Magnetic_Calc() :PI(3.14159265358979323846264338328){		//コンストラクタ．変数の初期化をしている．
	...
	areaWidth_output = 60.0e-3;		//計算領域 X 方向長さ
	areaDepth_output = 60.0e-3;		//計算領域 Y 方向長さ　
	areaHeight_output = 60.0e-3;	//計算領域 Z 方向長さ

	output_coil[0].incline_XY = 45 * (PI / 180);	// コイルの傾き角度 (ラジアンに変換)
	output_coil[0].incline_XZ = 35.26 * (PI / 180);	// コイルの傾き角度 (ラジアンに変換)
	output_coil[0].coil_pos_x = -32.4e-3;			//コイル位置　X 方向   先端の中心軸上
	output_coil[0].coil_pos_y = -32.4e-3;			//コイル位置　Y 方向   先端の中心軸上
	output_coil[0].coil_pos_z = -32.4e-3;			//コイル位置　Z 方向   先端の中心軸上	

	output_coil[1].incline_XY = 135 * (PI / 180);	// コイルの傾き角度 (ラジアンに変換)
	output_coil[1].incline_XZ = 35.26 * (PI / 180);	// コイルの傾き角度 (ラジアンに変換)
	output_coil[1].coil_pos_x = 92.4e-3;			//コイル位置　X 方向   先端の中心軸上
	output_coil[1].coil_pos_y = -32.4e-3;			//コイル位置　Y 方向   先端の中心軸上
	output_coil[1].coil_pos_z = -32.4e-3;			//コイル位置　Z 方向   先端の中心軸上	

	output_coil[2].incline_XY = 225 * (PI / 180);	// コイルの傾き角度 (ラジアンに変換)
	output_coil[2].incline_XZ = 35.26 * (PI / 180);	// コイルの傾き角度 (ラジアンに変換)
	output_coil[2].coil_pos_x = 92.4e-3;			//コイル位置　X 方向   先端の中心軸上
	output_coil[2].coil_pos_y = 92.4e-3;			//コイル位置　Y 方向   先端の中心軸上
	output_coil[2].coil_pos_z = -32.4e-3;			//コイル位置　Z 方向   先端の中心軸上	

	output_coil[3].incline_XY = 315 * (PI / 180);	// コイルの傾き角度 (ラジアンに変換)
	output_coil[3].incline_XZ = 35.26 * (PI / 180);	// コイルの傾き角度 (ラジアンに変換)
	output_coil[3].coil_pos_x = -32.4e-3;			//コイル位置　X 方向   先端の中心軸上
	output_coil[3].coil_pos_y = 92.4e-3;			//コイル位置　Y 方向   先端の中心軸上
	output_coil[3].coil_pos_z = -32.4e-3;			//コイル位置　Z 方向   先端の中心軸上		
	...
	WidthNum_output = (int)(areaWidth_output / ElmSize_output + 0.5);
	DepthNum_output = (int)(areaDepth_output / ElmSize_output + 0.5);
	HeightNum_output = (int)(areaHeight_output / ElmSize_output + 0.5);
