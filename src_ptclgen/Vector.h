...
namespace dem
{
	...
	struct Vector2D
	{
		...
		constexpr Vector2D Clamped(
			const Vector2D& Min, const Vector2D& Max) const noexcept
		{
			return
			{
		...
		{
			return Onto.LengthSq() 
				? Onto * Dot(Onto) / Onto.LengthSq() : Zero();
		}
	...
	inline std::basic_ostream<CharType>& operator <<(
		std::basic_ostream<CharType>& OUT, const Vector2D<Type>& V)
	{
	...
	struct Vector3D
	{
		...
		constexpr Vector3D Clamped(
			const Vector3D& Min, const Vector3D& Max) const noexcept
		{
			return
			{
				 x < Min.x ? Min.x : x > Max.x ? Max.x : x,
		...
		Vector3D RotatedAng(
			const Vector3D& Unit_Vec3, const value_type Ang) const noexcept
		{
			constexpr value_type r = 3.14159 / 180.0;
		...
		Vector3D RotatedRad(
			const Vector3D& Unit_Vec3, const value_type Rad) const noexcept
		{
	...
	inline constexpr Vector3D<Type> operator +(U S, const Vector3D<Type>& V) noexcept
	{
		return
		{ 
			static_cast<Type>(S) + V.x, 
			static_cast<Type>(S) + V.y, 
	...
	template <class CharType, class Type>
	inline std::basic_ostream<CharType>& operator <<(
		std::basic_ostream<CharType>& OUT, const Vector3D<Type>& V)
	{
